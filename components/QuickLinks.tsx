...
const QuickLinks: React.FC = () => {
  ...
  return (
    <div className="flex flex-col flex-1 shrink basis-0">
      <h2 className="text-[19.2px] font-medium leading-none text-white">Quick Links</h2>
      <nav className="flex flex-wrap gap-8 sm:gap-10 md:gap-12 lg:gap-16 items-start mt-6 w-full text-base text-zinc-500">
        {[linkItems.slice(0, 3), linkItems.slice(3)].map((column, columnIndex) => (
          <div key={columnIndex} className="flex flex-col items-start justify-start self-stretch">
            {column.map((item, index) => (
              <div
                key={index}
                className="flex text-[14px] relative flex-col justify-center whitespace-nowrap min-h-[24px] mt-4 first:mt-0"
              >
